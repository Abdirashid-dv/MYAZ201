...
public class ProductsController : ControllerBase
{
    ...
    public IActionResult CreateAll([FromBody] List<Product> products)
    {
        ...
        return Created($"/api/products/{products.Last().ProductId}", products.Last()); // 201
    }
    ...
    // endpoint which increase or decrease the price of all products by rate
    // api/products/price?rate=0.1
    [HttpPut("price")]
    public IActionResult UpdateAllPrice([FromQuery] double rate)
    {
        var products = _context.Products.ToList();
        foreach (var product in products)
        {
            product.Price += (int)(product.Price * rate);
        }
        _context.SaveChanges();
